...
const isProjectMediaCollection = (
  value: unknown,
): value is ProjectMediaCollection =>
  value === "sitePhotos" || value === "architecturalPlans";
...
const getProjectFiles = (files: Express.Multer.File[]) => {
  const sitePhotoFiles = files.filter(
    (file) => file.fieldname !== "architecturalPlans",
  );
  const architecturalPlanFiles = files.filter(
    (file) => file.fieldname === "architecturalPlans",
  );
...
const uploadProjectFiles = async (files: Express.Multer.File[]) => {
  ...
  const [sitePhotoUploads, architecturalPlanUploads] = await Promise.all([
    Promise.all(
      sitePhotoFiles.map((file) =>
        uploadImage(file, "inkingi/projects/site-photos"),
      ),
    ),
    Promise.all(
      architecturalPlanFiles.map((file) =>
        uploadImage(file, "inkingi/projects/architectural-plans"),
      ),
    ),
  ]);
...
const getMediaPublicIds = (value: Prisma.JsonValue | null | undefined) => {
...
const buildProjectUpdateData = (body: Record<string, unknown>) => {
  ...
  if (body.budget !== undefined) data.budget = String(body.budget);
...
const getNextProjectStatus = (status: ProjectStatus) => {
...
export const updateProject = async (req: Request, res: Response) => {
  try {
    ...
    if (uploaded.sitePhotos.length > 0) {
      ...
      data.sitePhotos = bodySitePhotos as Prisma.InputJsonValue;
    }
    ...
    if (uploaded.architecturalPlans.length > 0) {
      ...
      data.architecturalPlans = bodyArchitecturalPlans as Prisma.InputJsonValue;
    }
...
export const changeProjectImage = async (req: Request, res: Response) => {
  try {
    ...
    const uploadedImage = await uploadImage(
      file,
      getProjectMediaFolder(collection),
    );
