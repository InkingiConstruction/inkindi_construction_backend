...
export class MilestoneService {
  ...
  static async createMilestone(body: any, userId: string, userRole: string) {
    ...
    const engineerId =
      userRole === "admin" ? project.engineerId || userId : userId;
    ...
    const nextBudgetPercentage =
      budgetPercentage === undefined ? 0 : Number(budgetPercentage);
    const nextTotal = project.milestones.reduce(
      (sum, m) => sum + Number(m.budgetPercentage),
      nextBudgetPercentage,
    );
    if (nextTotal > 100)
  ...
  static async deleteMilestone(
    ...
    const hasDependent = Object.values(milestone._count).some(
      (count) => count > 0,
    );
    if (hasDependent)
  ...
  static async releaseMilestoneFunds(
    ...
    return prisma
      .$transaction(async (tx) => {
        ...
        const isSupervisor =
          approverRole === "supervisor" || approverRole === "admin";
        if (!isProjectOwner && !isSupervisor) {
          throw new Error(
            "Forbidden: only client or supervisor can release funds",
          );
        }
