...
import { WalletService } from "../escrow/escrow.service.js";
import {
...
const sanitizeRole = (role: unknown) => {
  const nextRole =
    typeof role === "string" ? role.trim().toLowerCase() : "client";
  ...
  try {
    ...
    if (!user) {
      return res
        ...
        .json({
          message:
            "This email has not been registered yet. Go back and create the account first.",
        });
    }
