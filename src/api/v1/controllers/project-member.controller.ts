...
export const updateProjectMember = async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({
        message:
          "Only the project owner, assigned engineer, or admin can update this assignment",
      });
    }
...
export const acceptProjectMember = async (req: Request, res: Response) => {
  try {
    const id = getParamId(req.params.id);
...
export const rejectProjectMember = async (req: Request, res: Response) => {
  try {
    const id = getParamId(req.params.id);
...
export const deleteProjectMember = async (req: Request, res: Response) => {
  try {
    ...
    if (
      !canManageProjectMember(assignment.project, req.user.id, req.user.role)
    ) {
      return res.status(403).json({
        message:
          "Only the project owner, assigned engineer, or admin can remove this assignment",
      });
    }
