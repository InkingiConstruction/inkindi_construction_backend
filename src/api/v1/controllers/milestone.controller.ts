...
const canReadProjectMilestones = (
  project: {
    clientId: string;
...
const isMilestoneStatus = (value: unknown): value is MilestoneStatus =>
  typeof value === "string" &&
  Object.values(MilestoneStatus).includes(value as MilestoneStatus);

const buildMilestoneUpdateData = (body: Record<string, unknown>) => {
  ...
  try {
    ...
    if (req.user.role !== "admin" && project.engineerId !== req.user.id) {
      return res.status(403).json({
        message:
          "Only the accepted project engineer or admin can create milestones",
      });
    }
    ...
    const nextTotal = project.milestones.reduce(
      (sum, milestone) => sum + Number(milestone.budgetPercentage),
...
export const getMilestoneById = async (req: Request, res: Response) => {
  try {
    ...
    if (
      !canReadProjectMilestones(milestone.project, req.user.id, req.user.role)
    ) {
...
export const updateMilestone = async (req: Request, res: Response) => {
  try {
    ...
    if (!canManageMilestone(existingMilestone, req.user.id, req.user.role)) {
      return res.status(403).json({
        message:
          "Only the project engineer, assigned supervisor, or admin can update this milestone",
      });
    }
    ...
    if (req.body.budgetPercentage !== undefined) {
      const nextTotal = existingMilestone.project.milestones.reduce(
        (sum, milestone) =>
          sum +
          (milestone.id === existingMilestone.id
            ? Number(req.body.budgetPercentage)
            : Number(milestone.budgetPercentage)),
        0,
      );
...
export const deleteMilestone = async (req: Request, res: Response) => {
  try {
    ...
    if (!canDeleteMilestone(milestone, req.user.id, req.user.role)) {
      return res.status(403).json({
        message:
          "Only the milestone engineer or admin can delete this milestone",
      });
    }

    const hasDependentRecords =
      milestone._count.inspections > 0 ||
