...
const isPurchaseOrderStatus = (
  ...
  typeof value === "string" &&
  Object.values(PurchaseOrderStatus).includes(value as PurchaseOrderStatus);
...
const canReadPurchaseOrder = (
  ...
  if (role === "supplier") return purchaseOrder.supplierId === userId;
  if (purchaseOrder.rfq.engineerId === userId) return true;
  if (purchaseOrder.rfq.project.clientId === userId) return true;
  if (purchaseOrder.rfq.project.engineerId === userId) return true;
  return Boolean(
    purchaseOrder.rfq.project.projectMembers?.some(
...
export const updatePurchaseOrder = async (req: Request, res: Response) => {
  try {
    ...
    const supplierAccepting =
      req.user.role === "supplier" &&
      existingPurchaseOrder.supplierId === req.user.id &&
      status === "accepted";
    const engineerUpdating =
      req.user.role === "admin" ||
    ...
    if (!supplierAccepting && !engineerUpdating) {
      return res.status(403).json({
        message:
          "Only supplier can accept, and RFQ engineer or admin can update purchase order",
      });
    }
