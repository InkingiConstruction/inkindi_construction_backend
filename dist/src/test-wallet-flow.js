...
const escrow_service_1 = require("./api/v1/escrow/escrow.service");
const milestone_service_1 = require("./api/v1/milestones/milestone.service");
...
async function main() {
    ...
    if (!client)
        throw new Error("No client user found — seed first");
    const project = await prisma.project.findFirst({
        where: { clientId: client.id },
        include: { escrowAccount: true, milestones: true },
    });
    if (!project)
        throw new Error("No project for this client");
    ...
    const wallet = await escrow_service_1.WalletService.ensureWallet(client.id);
    ...
    const confirmed = await escrow_service_1.WalletService.simulateFundingSuccess(funding.id);
    ...
    const transfer = await escrow_service_1.WalletService.transferToVault({
        userId: client.id,
    ...
    const summary = await escrow_service_1.WalletService.getWallet(client.id);
    ...
    const vaults = await escrow_service_1.WalletService.listUserProjectVaults(client.id);
    ...
    const details = await escrow_service_1.WalletService.getProjectVaultBalance(client.id, escrow.id);
    ...
    if (milestone) {
        ...
        if (currentEscrow && currentEscrow.balance.lt(50000)) {
            await escrow_service_1.WalletService.transferToVault({
                userId: client.id,
                escrowAccountId: escrow.id,
                amount: 100000,
        ...
        const release = await milestone_service_1.MilestoneService.releaseMilestoneFunds(milestone.id, client.id, "client");
