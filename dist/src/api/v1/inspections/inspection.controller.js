...
const getInspectionFiles = (files) => {
...
const getInspectionPhotos = (value) => {
...
const canReadMilestoneInspection = (milestone, userId, role) => {
    ...
    if (milestone.project.clientId === userId)
        return true;
    if (milestone.project.engineerId === userId)
        return true;
    return Boolean(milestone.project.projectMembers?.some((member) => member.userId === userId && member.status === "accepted"));
};
const canInspectMilestone = (milestone, userId, role) => {
    if (role === "admin")
        return true;
    if (role !== "supervisor")
        return false;
...
const milestoneStatusForDecision = (decision) => {
    if (decision === "approved")
        return "awaiting_client_payment";
    if (decision === "revision_required")
        return "revision_required";
    return undefined;
};
const buildInspectionUpdateData = (body, uploadedPhotos, signatureUrl, existingPhotos = []) => {
    const data = {};
...
const createInspection = async (req, res) => {
    try {
        ...
        const [photoUploads, signatureUpload] = await Promise.all([
            Promise.all(photoFiles.map((file) => uploadImage(file, "inkingi/inspections/photos"))),
            signatureFile
        ...
        const inspection = await db_js_1.default.$transaction(async (tx) => {
...
const getInspections = async (req, res) => {
    try {
        ...
        const inspections = await db_js_1.default.inspection.findMany({
            where: {
...
const getInspectionById = async (req, res) => {
    try {
        ...
        const inspection = await db_js_1.default.inspection.findUnique({
            where: { id },
            include: {
                milestone: {
                    include: {
...
const updateInspection = async (req, res) => {
    try {
        ...
        const existingInspection = await db_js_1.default.inspection.findUnique({
            where: { id },
            include: {
                milestone: {
                    include: {
...
const deleteInspection = async (req, res) => {
    try {
        ...
        await db_js_1.default.inspection.delete({
            where: { id },
        });
