...
const canReadDelivery = (delivery, userId, role) => {
    if (role === "admin")
        return true;
    if (role === "supplier")
        return delivery.supplierId === userId;
    if (delivery.purchaseOrder.rfq.engineerId === userId)
        return true;
    if (delivery.purchaseOrder.rfq.project.clientId === userId)
        return true;
    if (delivery.purchaseOrder.rfq.project.engineerId === userId)
        return true;
    return Boolean(delivery.purchaseOrder.rfq.project.projectMembers?.some((member) => member.userId === userId && member.status === "accepted"));
};
const getStatusDates = (status) => ({
    startedAt: status === "in_transit" || status === "delivered"
        ? new Date()
        : undefined,
    arrivedAt: status === "delivered" || status === "pending_confirmation"
        ? new Date()
        : undefined,
...
export const createDelivery = async (req, res) => {
    try {
        ...
        const activeDelivery = purchaseOrder.deliveries.find((delivery) => delivery.status !== "confirmed" && delivery.status !== "rejected");
        if (activeDelivery) {
        ...
        const uploads = await Promise.all(files.map((file) => uploadImage(file, "inkingi/procurement/deliveries")));
...
export const updateDelivery = async (req, res) => {
    try {
        ...
        const supplierUpdating = req.user.role === "supplier" && existingDelivery.supplierId === req.user.id;
        const projectApproving = req.user.role === "admin" ||
        ...
        if (!supplierUpdating && !projectApproving) {
            return res.status(403).json({
                message: "Only supplier can update delivery progress, and client, engineer or admin can confirm or reject delivery",
            });
        }
        if (supplierUpdating &&
        ...
        const delivery = await prisma.$transaction(async (tx) => {
            const updatedDelivery = await tx.delivery.update({
                ...
                data: {
                    status,
                    startGps: startGps !== undefined ? parseJsonField(startGps) : undefined,
                    endGps: endGps !== undefined ? parseJsonField(endGps) : undefined,
                    proofPhotos: uploadedProofPhotos.length > 0
