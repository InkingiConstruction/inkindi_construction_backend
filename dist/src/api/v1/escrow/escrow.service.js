 ...
 */
const toDecimal = (v) => v instanceof client_1.Prisma.Decimal ? v : new client_1.Prisma.Decimal(v);
// ─────────────────────────────────────────────────────────────────────────────
//  WALLET SERVICE
// ─────────────────────────────────────────────────────────────────────────────
class WalletService {
     ...
     */
    static async ensureWallet(userId, currency = "RWF") {
        const existing = await db_js_1.default.wallet.findUnique({ where: { userId } });
        if (existing)
            return existing;
        const wallet = await db_js_1.default.wallet.create({
            data: { userId, currency, balance: 0, status: "active" },
        });
        await wallet_queue_js_1.walletQueue.add("wallet.created", {
            userId,
            walletId: wallet.id,
        });
        return wallet;
    ...
    static async getWallet(userId) {
        const wallet = await db_js_1.default.wallet.findUnique({
            where: { userId },
            include: {
        ...
        if (!wallet)
            return null;
        const aggregates = await db_js_1.default.walletTransaction.aggregate({
            where: { walletId: wallet.id, status: "completed" },
            _sum: { amount: true },
        });
        const totalInVaults = await db_js_1.default.transaction.aggregate({
        ...
        return {
            ...
            totalInProjectVaults: totalInVaults._sum.amount ?? new client_1.Prisma.Decimal(0),
            netFlow: aggregates._sum.amount ?? new client_1.Prisma.Decimal(0),
        };
    }
    // ───────────────────────────────────────────────────────────────────────
    ...
    static async createFundingRequest(params) {
        ...
        return db_js_1.default.fundingRequest.create({
            data: {
                walletId: wallet.id,
     ...
     */
    static async confirmFunding(fundingRequestId, providerRef) {
        ...
        await wallet_queue_js_1.walletQueue.add("wallet.funded", {
     ...
     */
    static async simulateFundingSuccess(fundingRequestId) {
    ...
    static async transferToVault(params) {
        ...
        await wallet_queue_js_1.walletQueue.add("wallet.vault_transfer", {
            userId: params.userId,
     ...
     */
    static async getProjectVaultBalance(userId, escrowAccountId) {
        ...
        if (!escrow)
            return null;
        const totalDeposited = escrow.transactions.reduce((sum, t) => sum.plus(t.amount), new client_1.Prisma.Decimal(0));
        return {
            escrowAccountId: escrow.id,
    ...
    static async listUserProjectVaults(userId) {
        ...
        return escrows.map((e) => {
            const yourDeposits = e.transactions
                ...
                .reduce((s, t) => s.plus(t.amount), new client_1.Prisma.Decimal(0));
     ...
     */
    static async getWalletHistory(userId, opts = {}) {
        const { page = 1, limit = 20, type } = opts;
        const wallet = await db_js_1.default.wallet.findUnique({ where: { userId } });
        if (!wallet)
        ...
        const [items, total] = await Promise.all([
            db_js_1.default.walletTransaction.findMany({
                where,
                orderBy: { createdAt: "desc" },
                skip: (page - 1) * limit,
                take: limit,
            ...
            db_js_1.default.walletTransaction.count({ where }),
        ]);
     ...
     */
    static async setWalletStatus(userId, status, reason) {
        const wallet = await db_js_1.default.wallet.update({
            where: { userId },
            data: {
                status,
                ...(status === "frozen"
        ...
        await wallet_queue_js_1.walletQueue.add(status === "frozen" ? "wallet.frozen" : "wallet.unfrozen", {
            walletId: wallet.id,
