...
const crudRoutes = modelRoutes.flatMap((route) => [
    { method: "post", path: route.base, tag: route.tag, summary: `Create ${route.name}`, roles: route.roles.create, multipart: route.multipart },
    { method: "get", path: route.base, tag: route.tag, summary: `List ${route.name} records`, roles: route.roles.read, query: route.query },
    { method: "get", path: `${route.base}/{id}`, tag: route.tag, summary: `Get ${route.name} by ID`, roles: route.roles.read },
    { method: "put", path: `${route.base}/{id}`, tag: route.tag, summary: `Update ${route.name}`, roles: route.roles.update, multipart: route.multipart },
    { method: "delete", path: `${route.base}/{id}`, tag: route.tag, summary: `Delete ${route.name}`, roles: route.roles.delete },
]);
const routes = [...authRoutes, ...customRoutes, ...crudRoutes];
/* --------------------------------------------------------------------------
 * 4. HELPERS
 * ------------------------------------------------------------------------ */
const buildPropertiesFromExample = (body) => {
    if (!body)
        return undefined;
    const properties = {};
...
const createParameters = (route) => {
    const pathParams = [...route.path.matchAll(/{([^}]+)}/g)].map((m) => ({
        name: m[1],
        in: "path",
        required: true,
        schema: { type: "string" },
...
const getResponseSchema = (tag, summary) => {
    if (tag === "Wallet") {
        if (summary.includes("summary"))
            return { $ref: "#/components/schemas/WalletSummary" };
        if (summary.includes("List all project vaults"))
            return {
                type: "object",
                properties: { items: { type: "array", items: { $ref: "#/components/schemas/ProjectVaultSummary" } } },
            };
        if (summary.includes("Get details for a specific project vault"))
            return { $ref: "#/components/schemas/ProjectVaultDetails" };
        if (summary.includes("history"))
            return {
                type: "object",
                properties: {
        ...
        if (summary.includes("funding request"))
            return { $ref: "#/components/schemas/FundingRequest" };
...
exports.openApiDocument = {
    ...
    info: {
        ...
        version: "1.0.0",
        description: "Versioned REST API documentation for the Inkingi Construction platform — projects, milestones, escrow, wallets, KYC, suppliers, disputes, RFQs.",
    },
    servers: [
...
exports.default = exports.openApiDocument;
